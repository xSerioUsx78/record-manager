...
const Index: React.FC<propsInterface> = ({ record, setRecord }) => {
  const [hasMore, setHasMore] = useState<boolean>(record.next ? true : false);

  const deleteRecordCallback = useCallback(
    async (id) => {
      try {
        axios().delete(requests.recordDetail(id));
  ...
  const handleFetchMoreData = async () => {
    if (record.next) {
      const { data }: any = await axios().get(record.next);
