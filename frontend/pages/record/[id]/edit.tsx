...
export const getServerSideProps: GetServerSideProps = loginRequiredSSR(wrapper.getServerSideProps(store => async (context: any) => {
  const id = context.params.id;
  let record!: recordResultsItemInterface | {};
  try {
    const state = store.getState();
    const token = state.auth.token;
    const res: responseDate = await authAxios(token!).get(requests.recordDetail(id));
    record = res.data;
  } catch(e) {
    console.log(e);
    record = {};
